...
- Media upload requires `media.upload` scope, base64 via MCP; MCP's own body limit (`MCP_JSON_BODY_LIMIT`, default 100 MB) is a backstop only — WordPress's real PHP limits are authoritative (see `wordpress_get_site`)
